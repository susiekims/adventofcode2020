...
const part1 = (inputs, preamble) => {
  for (let i = preamble; i < inputs.length; i++) {
    const isValid = findPairs([...inputs].slice(i - preamble, i), inputs[i]);
    if (!isValid) {
      return inputs[i];
    }
  }
...
const fs = require("fs");
const input = fs.readFileSync("day9/input.txt", "utf8").trim().split("\n").map(x => Number(x));

console.log(part1(input, 25)); // 1398413738

module.exports = { findPairs, part1, testInput, input };
