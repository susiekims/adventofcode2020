...
const getPosition = (code, lower, upper, totalRows) => {
  let min = 0;
  let max = totalRows;
...
const getSeatId = (code) => {
  ...
  const columns = rows.splice(7, 3);

  const row = getPosition(rows, "F", "B", 127);
  const column = getPosition(columns, "L", "R", 7);
...
module.exports = { getSeatId, input };
